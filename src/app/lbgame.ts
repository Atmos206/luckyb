  ...
  playNPC() {
    setTimeout(() => {
      ...
      if (currentPlayer.playerCards.hand.length > 0 || currentPlayer.playerCards.up.length > 0) {
        ...
        let stack =
          currentPlayer.playerCards.hand.length > 0 ? currentPlayer.playerCards.hand : currentPlayer.playerCards.up;
        ...
        currentPlayer.playerCards.hand.length == 0 &&
        currentPlayer.playerCards.up.length == 0 &&
        currentPlayer.playerCards.down.length > 0
      ) {
...
class Player {
  ...
  endPlayerTurn() {
    ...
    this.game.currentPlayerIndex =
      this.game.currentPlayerIndex == this.game.players.length - 1 ? 0 : this.game.currentPlayerIndex + 1;
    ...
    if (this.cardCheck(card)) {
      console.log(
        'PLAY: "' +
          ...
          card.suit +
          ' -- H' +
          this.playerCards.hand.length +
          ' U' +
          this.playerCards.up.length +
          ' D' +
          this.playerCards.down.length
      );
